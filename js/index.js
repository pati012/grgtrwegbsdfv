                ...
                for(let item = 0; item < spoller.length; item++){
                  ...
                  element.addEventListener('click', function (e){
                    const elementOpen = element.nextElementSibling;
                     if(!elementOpen.style.maxHeight == null){
                      document.querySelectorAll('.spoller__text').forEach(item => (item.style.maxHeight = null));
                     }
                     else{
                      document.querySelectorAll('.spoller__text').forEach(item => (item.style.maxHeight = null));
                      const scrollHieght = elementOpen.scrollHeight;
                       elementOpen.style.maxHeight = scrollHieght + 'px';
                     }
                  })
                }
              }